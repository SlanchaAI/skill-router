"""Canary recommendation: the production-honest alternative to the offline A/B.

Instead of scoring a fixed eval set, serve the challenger to a small fraction (epsilon) of live
traffic, judge each *real* outcome, and recommend promotion once the challenger's posterior beats
the champion's under a sequential Thompson-sampling decision. Activation still requires human UI
approval. This protects live traffic and gates on outcomes the optimizer never trained on.
...
request we estimate P(challenger > champion) by sampling both posteriors and stop early to recommend
promotion (P >= promote_p) or reject (P <= 1-promote_p) once both arms have min_samples.
...
from .promote import load_pending, save_pending
...
def run_canary(skill: str, challenger_file: str | None = None, epsilon: float = 0.25,
               ...
               success_thresh: float = 0.5, seed: int = 0, log=print) -> dict:
    usage_ledger.reset()
    ...
    for i in range(max_requests):
        ...
        if served["challenger"] >= min_samples and served["champion"] >= min_samples:
            ...
            if p >= promote_p:
                ...
                matching_pending = (
                    pending if pending and pending.get("challenger_components") == challenger else None
                )
                gate = {"promotable": True, "blocked": [], "warnings": []}
                recommendation = matching_pending or {
                    "kind": "canary",
                    "skill": skill,
                    ...
                    "changed_components": [
                        ...
                        if challenger.get(key, "").strip() != champion[key].strip()
                    ],
                    "gate": gate,
                    "evidence": {
                        "champion": {"revision": revisions["champion"]},
                        "challenger": {"revision": revisions["challenger"]},
                        "gate": gate,
                    },
                }
                save_pending(skill, {
...
if __name__ == "__main__":
    ...
    run_canary(args.skill, challenger_file=args.challenger_file, epsilon=args.epsilon,
               max_requests=args.max, min_samples=args.min_samples)
